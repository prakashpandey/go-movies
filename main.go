...
func CreateEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	bucket.Insert(uuidVal.String(), movie, 0)
	json.NewEncoder(w).Encode(movie)
}

func SearchEndpoints(w http.ResponseWriter, req *http.Request) {
	var movies []Movie
	params := mux.Vars(req)
...
func main() {
	...
	//cluster.Authenticate('root', 'rootroot')
	bucketName = "pcp-go-bucket"
	bucket, _ = cluster.OpenBucket(bucketName, "")
	router := mux.NewRouter()
	router.HandleFunc("/movies", ListEndpoint).Methods("GET")
	router.HandleFunc("/movies", CreateEndpoint).Methods("POST")
	router.HandleFunc("/search/{title}", SearchEndpoints).Methods("Get")
	log.Fatal(
		http.ListenAndServe(":8080", handlers.CORS (handlers.AllowedMethods([]string {"GET", "POST", "PUT", "HEAD"}), handlers.AllowedOrigins([]string {"*"}))(router)))
}
